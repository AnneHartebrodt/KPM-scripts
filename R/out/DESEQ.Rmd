...
```{r gene_expression}
pvals_raw<-fread("/home/anne/Documents/Master/MA/data/DESeq_out/DESeq_out_sub_merged_entrez.tsv", header=T, sep="\t")
...
pvals_raw[,median:=lapply(.SD,median), by = geneName]
#pvals_clean<-pvals_raw[which(pvals_raw$Mean!=1.0)]
pvals_clean<-pvals_raw[which(pvals_raw$median!=1.0)]
#write.table(pvals_clean[,1:(ncol(pvals_clean)-1)], "/home/anne/Documents/Master/MA/data/", sep = "\t", row.names = F, quote = F)
...
# invert file before reading
```{r nodeDist}
#system("tac home/anne/Documents/Master/MA/Testing/out_new/nodeDist/nodeDistFall.txt > /home/anne/Documents/Master/MA/Testing/out_new/nodeDist/nodeDistFall2.txt")
file <- fread("/home/anne/Documents/Master/MA/Testing/out_new/nodeDist/nodeDistFall2.txt", fill = T)
...
ggplot(fall[run %in% c("10","46", "96")], aes(x=value))+geom_histogram()+facet_wrap(c("V1"))+ ggtitle("Node degree distribution pooled over runs")
```
...
<!-- teststats<-fread("/home/anne/Documents/Master/MA/Testing/out_new/dist/pdistributionT.txt", header= FALSE, sep = "\t") -->
...
#plot(gpF)
#plot(pF)  
gridExtra::grid.arrange(gpF, pF)
for(i in 1:3){
build<-ggplot_build(gpF)$data[[1]]
#integrate.xy(build$x[which(is.finite(build$x))], build$y[which(is.finite(build$x))])
o<-order(build$x)
m<-max(which(build$y[o]<0.05))
xo<-build$x[o]
gpF<-gpF+geom_vline(xintercept = xo[m]) #0.63 0.22 0.11
}

gpF<-gpF+geom_vline(xintercept = 0.63)+geom_vline(xintercept = 0.22)+geom_vline(xintercept = 0.11)+geom_hline(yintercept = 0.05)
plot(gpF)
```
