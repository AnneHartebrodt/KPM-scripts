...
pvals<-fread("Masterarbeit/data/differential_out/HD/DESEQ/results.tsv")
...
base.dir<-file.path("/home/anne/Masterarbeit/batch_test/")
...
for (ind in 1:5) {
  ...
  nodiff <- lapply(nondifferential, function(x) sample(abs(pvals$pvalue[n:nrow(pvals)]), 8, replace = F))
