...
g0<-ggplot(dat, aes(x=V2, fill=file))+geom_histogram(position = "dodge", bins = 100)+ggtitle("Distribution of p-values in dataset")+xlab("P-Value")+ylab("Count")+guides(fill=F)+facet_wrap("dir", nrow = 1)
```
```{r, echo=F}
...
network_jaccard[, nr_networks:=sapply(network_jaccard$filename, function(x) length(gregexpr("_", x)[[1]]))]
```
