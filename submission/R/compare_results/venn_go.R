...
diff.pkpm<-setdiff(intersect(diff.go, probal.1.go), bionet.go)
diff.pkpm<-probal.1.go.file[V1 %in% diff.pkpm][order(V2)][1:20]
onto22<-diff.go.file[V1 %in% diff.pkpm][order(V2)][1:20]
#merge(onto11, onto22, by="V1", all=T)
fwrite(diff.pkpm,file=file.path(go.dir, "diff.pkpm.pretendwebdoesnotexist.tsv"), sep="\t")
...
bionet.only<-setdiff(setdiff(bionet.go,diff.go),probal.1.go)
bio.onl<-bionet.go.file[V1 %in% bionet.only][order(V2)]
fwrite(bio.onl,file=file.path(go.dir, "bionet.only.pretendwebdoesnotexist.tsv"), sep="\t")
